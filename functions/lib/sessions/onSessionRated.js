...
exports.onSessionRated = (0, firestore_1.onDocumentUpdated)({ document: "sessions/{sessionId}", region: constants_1.REGION }, async (event) => {
    ...
    const beforeRating = before.userRating;
    const afterRating = after.userRating;
    if (afterRating === undefined ||
        afterRating === null ||
        typeof afterRating !== "number") {
        return;
    }
    if (beforeRating !== undefined && beforeRating !== null)
        return;
    // Already aggregated — a snapshot replay or a follow-up write
    // after the flag was set. Bail out.
    if (after.ratingAggregated === true)
        return;
    ...
    try {
        await db.runTransaction(async (tx) => {
            ...
            const oldRating = (_b = priestData.rating) !== null && _b !== void 0 ? _b : 0;
            const oldCount = (_c = priestData.reviewCount) !== null && _c !== void 0 ? _c : 0;
            const lastMilestone = (_d = priestData.lastReviewMilestone) !== null && _d !== void 0 ? _d : 0;
        ...
        console.error(`[onSessionRated] Aggregation failed for ${sessionId}:`, err);
        return;
    }
    ...
    const userName = (_c = after.userName) !== null && _c !== void 0 ? _c : "Someone";
    const reviewTitle = "⭐ New Review";
    const reviewBody = `${userName} rated your session ${rating} ` +
        `star${rating === 1 ? "" : "s"}.`;
    try {
        ...
        console.error(`[onSessionRated] per-review inbox write failed for ${priestId}:`, err);
    }
    ...
    if (milestoneToPush !== null) {
        const milestone = milestoneToPush;
        ...
        try {
            ...
            console.error(`[onSessionRated] milestone inbox write failed for ${priestId}:`, err);
        }
    }
});
...
function buildMilestoneCopy(milestone, priestName) {
...
//# sourceMappingURL=onSessionRated.js.map