...
function getAndroidPublisher() {
    ...
    if (!raw) {
        ...
        throw new https_1.HttpsError("failed-precondition", "Play verification not configured on the server");
    }
    let credentials;
    try {
        ...
        throw new https_1.HttpsError("internal", "Play service account secret is not valid JSON");
    }
    if (!credentials.client_email || !credentials.private_key) {
        throw new https_1.HttpsError("internal", "Play service account secret is missing required fields");
    }
    ...
    cachedClient = googleapis_1.google.androidpublisher({ version: "v3", auth });
    return cachedClient;
}
...
async function verifyProductPurchase(args) {
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", `Could not verify purchase with Google Play: ${msg}`);
    }
    if (purchase.purchaseState !== 0) {
        throw new https_1.HttpsError("failed-precondition", "purchase_not_valid");
    }
    return purchase;
}
...
async function consumeProduct(args) {
    const client = getAndroidPublisher();
    try {
