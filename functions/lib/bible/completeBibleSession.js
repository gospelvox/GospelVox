...
exports.completeBibleSession = void 0;
...
exports.completeBibleSession = (0, https_1.onCall)({ region: constants_1.REGION }, async (request) => {
    var _a, _b, _c;
    if (!request.auth) {
    ...
    if (session.status !== "upcoming" && session.status !== "live") {
        throw new https_1.HttpsError("failed-precondition", `Cannot complete a ${session.status} session`);
    }
    ...
    try {
        ...
        console.error("[completeBibleSession] priest doc update failed for " +
            `priest=${callerUid} session=${sessionId}:`, err);
    }
    ...
    const allRegsSnap = await db
        ...
        .get();
    const paidCount = allRegsSnap.docs.filter((d) => d.data().status === "paid").length;
    const activeRegs = allRegsSnap.docs.filter((d) => d.data().status !== "cancelled");
    const price = Number((_b = session.price) !== null && _b !== void 0 ? _b : 0);
    ...
    try {
        ...
        await notifRef.set({
            ...
            body: `"${title}" — ₹${totalRevenue} earned from ${paidCount} ` +
        ...
        console.error("[completeBibleSession] priest notif write failed for " +
            `${sessionId}:`, err);
    }
    ...
    const userBody = `"${title}" has wrapped up. ` +
    ...
    for (let i = 0; i < activeRegs.length; i += FANOUT_CHUNK_SIZE) {
        ...
        try {
            ...
            console.error("[completeBibleSession] user notif batch failed for " +
...
//# sourceMappingURL=completeBibleSession.js.map