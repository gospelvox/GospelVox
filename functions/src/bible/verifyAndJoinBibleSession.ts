...
import {verifyProductPurchase, consumeProduct} from "../payments/playVerify";
...
export const verifyAndJoinBibleSession = onCall(
  ...
  async (request) => {
    ...
    const {sessionId, productId, verificationData} = request.data as {
      sessionId?: string;
      productId?: string;
    ...
    if (
      ...
      throw new HttpsError(
        "invalid-argument",
        "sessionId, productId, and verificationData are required",
      );
    }
    ...
    if (productId !== BIBLE_PRODUCT_ID) {
      throw new HttpsError(
        "failed-precondition",
        "Unsupported product for bible session",
      );
    }
    ...
    const startedTs =
      sessionData.startedAt as admin.firestore.Timestamp | undefined;
    if (startedTs) {
      const durationRaw = sessionData.durationMinutes as number | undefined;
      const durationMin =
      ...
      if (Date.now() > deadlineMs) {
        throw new HttpsError(
          "failed-precondition",
          "This session has ended — payment is closed.",
        );
      }
    }
    ...
    if (
      ...
      try {
        ...
        console.error(
          "[verifyAndJoinBibleSession] idempotent consume rescue failed:",
    ...
    if (purchaseSnap.exists) {
      const recordedSessionId = purchaseSnap.data()?.sessionId;
      if (recordedSessionId && recordedSessionId !== sessionId) {
        throw new HttpsError(
          "permission-denied",
          "Purchase token already used for a different session",
        );
      }
      try {
        await consumeProduct({productId, purchaseToken});
    ...
    try {
      ...
      console.error(
        "[verifyAndJoinBibleSession] commission config read failed; " +
          `using default ${DEFAULT_BIBLE_COMMISSION_PERCENT}%:`,
        err,
      );
    }
    const priestEarning = Math.floor(
      BIBLE_PRICE_RUPEES * (1 - bibleCommissionPercent / 100),
    );
    ...
    try {
      ...
      console.error(
        "[verifyAndJoinBibleSession] dismiss prior live notif failed for " +
          `${sessionId} uid=${uid}:`,
        err,
      );
    }
